...
class DataSet:
    ...
    def __init__(self, filename, directory='', nxs2spec=False, alias_dict=None):
        ...
        if nxs2spec:
            ...
            try:  # try reading for h k l coordinates at the beginning of the scan.
                ...
                k_tmp = f.sample_info._f_get_child('i14-c-cx2-ex-diff-uhv-k')._f_get_child('position_pre').read()[0]
            ...
            try:
                ...
                p6 = str(gamma)
                p7 = str(gamma)
                self._nxs2spec.P_line = ('#P0 ' + p1 + ' ' + p2 + ' ' + p3 + ' ' + p4 +
                                         ' ' + p5 + ' ' + p6 + ' ' + p7 + '\n')
                p0 = True
            except tables.NoSuchNodeError:
                pass
    ...
    def calc_roi(self, stack, roiextent, maskname, attcoef, filters, acq_time, roi_name):
    ...
    def calc_roi_new2(self):
        ...
        rejected."""
        list2d = self._list2d
        common_roots = ['_roi_limits', '_roi_names', '_ifmask']  # common root names for attributes
        ...
        if self.scantype == 'SBS':  # SBS Correction
            ...
            if not possible:
                print('No correction applied: missing  some data.... check for detector,'
                ...
                if not hasattr(self, '_npts'):  # check if the process was alredy runned once on this object
                    ...
                    for el in list2d:
                        ...
                        for pos, roi in enumerate(self.__getattribute__('_roi_limits_' + el), start=0):
                            roi_name = self.__getattribute__('_roi_names_' + el)[pos] + '_' + el + 'c_new'
                            stack = self.__getattribute__(el)
                            attenuators = self.att_sbs_xpad[:]
                            self.calc_roi(stack, roi, maskname, self._coef, attenuators,
                                          self._integration_time, roi_name)
                    
        if self.scantype == 'FLY':  # FLY correction
            possible = possible * hasattr(self, 'attenuation') 
            if not possible:
                print('No correction applied: missing  some data, check for xpad,'
