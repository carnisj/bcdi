    ...
    def __init__(self, filename, directory='', Nxs2Spec = False, alias_dict=None):
        ...
        try:
            ...
            for el in det:
                ...
                #print(np.shape(detarray))
                if np.shape(detarray)[1] == 120:
                    #self.__delattr__(el)   # keeping the name generated by the flyscan for MaskTool
                    self.xpad70 = detarray
                    #self.attlist.remove(el)
                    self.attlist.append('xpad70')
                    self._list2D.append('xpad70')
                if np.shape(detarray)[1] == 240:
                    #self.__delattr__(el)   # keeping the name generated by the flyscan for MaskTool
                    self.xpad140 = detarray
                    #self.attlist.remove(el)
                    self.attlist.append('xpad140')
                    self._list2D.append('xpad140')

        except:
            print ('xpads not in use')
