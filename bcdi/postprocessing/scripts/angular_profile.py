...
if upsampling_factor > 1:
    ...
    valid.valid_container(voxel_size, container_types=(list, tuple, np.ndarray), length=ndim, item_types=Real,
                          min_excluded=0, name='angular_profile')
...
if ndim == 3:
    ...
    for idx, thres in enumerate(threshold):
        ...
        for idy, direction in enumerate(directions):
            ...
            distance, cut = util.linecut(array=tmp_obj, point=origin, direction=direction, voxel_size=voxel_size)
...
#################################################################################################
    ...
    for idx, direction in enumerate(directions):
        ...
        result[f'direction ({direction[0]:.4f},{direction[1]:.4f})'] =\
            {'angle': angles[idx], 'distance': distance, 'cut': cut, 'threshold': threshold, 'width': width}
