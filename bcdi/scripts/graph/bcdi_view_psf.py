...
if width is not None and isinstance(width, Real):
    width = (width,)*3
valid.valid_container(width, container_types=(tuple, list, np.ndarray), length=3, item_types=int, min_excluded=0,
...
if any((cen_z-width[0] < 0, cen_z+width[0] > nbz,
...
#########################
fig, _, _ = gu.multislices_plot(dataset, scale=plot_scale, sum_frames=False, title=title, vmin=vmin, vmax=vmax,
                                reciprocal_space=False, is_orthogonal=is_orthogonal,
                                plot_colorbar=True, width_z=2*width[0], width_y=2*width[1], width_x=2*width[2])
...
if save_slices:
    ...
    fig, ax, _ = gu.imshow_plot(dataset[cen_z, :, :], sum_frames=False, width_v=2*width[1], width_h=2*width[2],
                                scale=plot_scale, vmin=vmin, vmax=vmax, reciprocal_space=False,
                                is_orthogonal=is_orthogonal, plot_colorbar=True)
    ...
    gu.savefig(savedir=savedir, figure=fig, axes=ax, tick_width=tick_width, tick_length=tick_length,
               ...
               titles=fname+' central slice in '+labels[0][2], title_size=20,
               legend_labelsize=14, filename=fname+'_centralslice_z'+comment)

    fig, ax, _ = gu.imshow_plot(dataset[:, cen_y, :], sum_frames=False, width_v=2*width[0], width_h=2*width[2],
                                scale=plot_scale, vmin=vmin, vmax=vmax, reciprocal_space=False,
                                is_orthogonal=is_orthogonal, plot_colorbar=True)

    gu.savefig(savedir=savedir, figure=fig, axes=ax, tick_width=tick_width, tick_length=tick_length,
               tick_labelsize=16, xlabels=labels[1][0], ylabels=labels[1][1], label_size=20,
               titles=fname+' central slice in '+labels[1][2], title_size=20,
               legend_labelsize=14, filename=fname+'_centralslice_y'+comment)

    fig, ax, _ = gu.imshow_plot(dataset[:, :, cen_x], sum_frames=False, width_v=2*width[0], width_h=2*width[1],
                                scale=plot_scale, vmin=vmin, vmax=vmax, reciprocal_space=False,
                                is_orthogonal=is_orthogonal, plot_colorbar=True)

    gu.savefig(savedir=savedir, figure=fig, axes=ax, tick_width=tick_width, tick_length=tick_length,
               tick_labelsize=16, xlabels=labels[2][0], ylabels=labels[2][1], label_size=20,
               titles=fname+' central slice in '+labels[2][2], title_size=20,
               legend_labelsize=14, filename=fname+'_centralslice_x'+comment)
